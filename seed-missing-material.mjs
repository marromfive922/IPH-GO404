...
try {
  // Update material for Protocolo de Comunicação
  await connection.execute(
    'UPDATE disciplines SET material = ? WHERE slug = ?',
    [protocoloMaterial, 'protocolo-comunicacao']
  );
  console.log('✓ Material adicionado para Protocolo de Comunicação!');

  // Update material for Sistemas Operativos
  await connection.execute(
    'UPDATE disciplines SET material = ? WHERE slug = ?',
    [soMaterial, 'sistemas-operativos']
  );
  console.log('✓ Material adicionado para Sistemas Operativos!');

  // Update material for Arquitetura de Computadores
  await connection.execute(
    'UPDATE disciplines SET material = ? WHERE slug = ?',
    [arquiteturaMaterial, 'arquitetura-computadores']
  );
